...
class ControleJeuxPiecesForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        kilometrage_jeu = self.cleaned_data.get("kilometres_chassis")

        if voiture and kilometrage_jeu is not None:
            # 🔒 Sécurité : ne jamais diminuer le kilométrage
            if kilometrage_jeu < voiture.kilometres_chassis:
                raise forms.ValidationError(
                    f"Le kilométrage du check-up ({kilometrage_jeu}) "
            ...
            if kilometrage_jeu > voiture.kilometres_chassis:
                voiture.kilometres_chassis = kilometrage_jeu
                voiture.save(update_fields=["kilometres_chassis"])

            # ✅ Mettre à jour le contrôle
            instance.kilometres_chassis = kilometrage_jeu
