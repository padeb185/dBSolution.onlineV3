...
@never_cache
@login_required
def delete_niveau_view(request, niveau_id):
    ...
    exemplaire = niveau.voiture_exemplaire
    maintenance = niveau.maintenance
    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                ...
                # USER LOG
                # ==================================================
                ACTION_SUPPRESSION_NIVEAU = gettext_noop(
                    "Suppression du contrôle des niveaux"
                )

                UserLog.objects.create(
                    utilisateur=request.user,
                    action=(
                        f"{ACTION_SUPPRESSION_NIVEAU} - "
                        f"{immatriculation}"
                    )
                )
            ...
            messages.success(
                request,
                _("Contrôle des niveaux supprimé avec succès.")
            )
...
def niveau_pdf_view(request, niveau_id):
    ...
    nom_fichier = (
        f"{_('Niveaux')}_{technicien}_{immatriculation}_{date_pdf}.pdf"
    )
