                ...
                if km is None:
                    ...
                    if km < ancien_kilometrage:
                        ...
                        with transaction.atomic():
                            ...
                            refroidissement.voiture_exemplaire = (
                                exemplaire
                            )
            ...
            try:
                with transaction.atomic():
                    ...
                    if km is not None:
                        ...
                        refroidissement.kilometrage_variation = 0
...
@never_cache
@login_required
def delete_ref_view(request, ref_id):
    ...
    if request.method == "POST":
        ...
        try:
            with transaction.atomic():
                ...
                # USER LOG
                # ==================================================
                ACTION_SUPPRESSION_REF = gettext_noop(
                    "Suppression du contrôle du refroidissement"
                )

                UserLog.objects.create(
                    utilisateur=request.user,
                    action=(
                        f"{ACTION_SUPPRESSION_REF} - "
                        f"{immatriculation}"
                    )
                )
            ...
            messages.success(
                request,
                _("Contrôle du refroidissement supprimé avec succès.")
            )
