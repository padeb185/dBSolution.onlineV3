...
# Modèle fusionné
# ---------------------------
class ControleBteAuto(TechnicienMixin, models.Model):
    ...
    kilometrage_bte_auto = models.PositiveIntegerField(
    ...
    # --- Boîte Automatique ---
    auto_emb_convertisseur_couple = models.CharField(max_length=25, choices=BoiteVitesseEtat.choices, default=BoiteVitesseEtat.OK, verbose_name=_("Convertisseur de couple"))
    auto_emb_embrayages_auto = models.CharField(max_length=25, choices=BoiteVitesseEtat.choices,default=BoiteVitesseEtat.OK, verbose_name=_("Embrayages automatiques"))
    ...
    pompes_valves = models.CharField(max_length=25, choices=BoiteVitesseEtat.choices,default=BoiteVitesseEtat.OK, verbose_name=_("Valves de contrôle"))
    ...
    class Meta:
        verbose_name = _("Contrôle boite automatique")
        verbose_name_plural = _("Contrôles boites automatiques")
    ...
    def clean(self):
        super().clean()
        if self.voiture_exemplaire and self.kilometrage_bte_auto is not None:
            if self.kilometrage_bte_auto < self.voiture_exemplaire.kilometres_chassis:
                raise ValidationError({
                    'kilometrage_bte_auto': _(
                        f"Le kilométrage du contrôle ({self.kilometrage_bte_auto}) "
    ...
    def save(self, *args, **kwargs):
        ...
        if self.voiture_exemplaire and self.kilometrage_bte_auto:
            if self.kilometrage_bte_auto > self.voiture_exemplaire.kilometres_chassis:
                self.voiture_exemplaire.kilometres_chassis = self.kilometrage_bte_auto
